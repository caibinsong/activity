(function($) {
	...
	var utils = {
		...
		elToDocOffsetDis:function(el){			//6.7  无论元素处于固定定位还是绝对定位的场景，都可以算出其相对对应场景的位移
		...
		createClass(Datepicker, [{
			...
			value: function output(dateStr) {
				var self = this,
					...
					$datepickerBox = null,
					eleTop = utils.elToDocOffsetDis($element[0]).top,	//6.7 获取调用元素相对于根节点上边框的差
					eleLeft = utils.elToDocOffsetDis($element[0]).left,	//6.7 获取调用元素相对于根节点左边框的差
